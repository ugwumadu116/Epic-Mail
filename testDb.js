...
connectDb.query(userGroupMessageTable)
  .then(() => {
    connectDb.query(inboxMessageTable);
  })
  .then(() => {
    connectDb.query(sentMessageTable);
  })
  .then(() => {
    connectDb.query(UserContactTable);
  })
  .then(() => {
    connectDb.query(groupMessageTable);
  })
  .then(() => {
    connectDb.query(messageTable);
  })
  .then(() => {
    connectDb.query(registeredUserTable);
  })
  .then(() => process.exit())
  .catch((err) => {
